...
export async function updateLink(ef: IExecuteFunctions) {
	try {
		// Obter parâmetros do nó
		const linkId = ef.getNodeParameter('id', 0) as string;
		...
		return await codeqrApiRequest.call(ef, 'PUT', `/links/${linkId}`, body);
	} catch (error) {
		const errorData = {
			success: false,
			error: {
				message: error.message,
				details: error.details || 'An error occurred while updating the link.',
